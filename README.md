...
目前项目正在全部重写，重写后的项目将并入[KalznOJ](https://github.com/Kalzncc/KalznOnlineJudge)项目，作为KalznOJ的评测沙箱。重写将着重实现评测机的安全配置，已经资源限制。
#### 就目前而言，本程序可以：
...
6、对待测进程进行权限控制。（这点很关键，因为没有做，所以没有安全性保证，只能称得上是玩具）

7、没有对解释型语言的支持。judger接受的只能是可执行文件。不过很好写，改变一下execl的参数，用解释器运行即可，有时间再写吧。

## 整体而言
只是Judger最核心，最基础的东西，想挂到服务器上是由还缺两到三层壳，最显而易见的是编译这层壳：通过语言选择编译器路径，然后判定语言类型（编译，解释，半编译半解释（java）），选择正确的评测方法。再上面一层是评测队列管理，最上面就是judge server的request处理了。
